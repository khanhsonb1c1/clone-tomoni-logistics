...
jQuery(function ($) {
  ...
  let interval = null;
  const countSelector = $(".single-counter > span, .single-card > h3");
  if (countSelector.length) {
    const startingTop = countSelector.offset().top - window.innerHeight;
    if (startingTop > 0) {
      $(window).on("scroll", function () {
  ...
  function count(counters, start, value, id) {
    ...
    const inc = incrementValue(value);
  ...
  function startCounting() {
    const counters = $(".single-counter > span, .single-card > h3");
  ...
  function submitGetQuoteForm() {
    ...
    const name = $('#getQuoteFrm input[name="name"]').val();
    const email = $('#getQuoteFrm input[name="email"]').val();
    const subject = $('#getQuoteFrm input[name="subject"]').val();
    const message = $('#getQuoteFrm textarea[name="message"]').val();
  ...
  function submitContactForm() {
    // Initiate Variables With Form Content
    const name = $("#contactForm #name").val();
    const email = $("#contactForm #email").val();
    const phone = $("#contactForm #phone").val();
    const company = $("#contactForm #company").val();
